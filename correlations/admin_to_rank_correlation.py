...
# calculate change in admission metrics for the previous year to the current year
# calculate change in rank from the current year to the next
...
# produce year by year correlation and p-value heatmaps
correlation_results = {}
...
for year in range(2019, 2023):
    ...
    if correlation_results:
        ...
        plt.title(f'Correlation and P-value Heatmap for Changes in Admission statistics {year-1}-{year} and Rank {year}-{year+1}')
        plt.show()
    else:
        print(f"No valid data for correlation in {year}")
